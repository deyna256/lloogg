use super::helpers::*;
use lloogg::{constants::*, snapshot::SnapshotManager, store::Store, time::now_ns};
...
fn test_snapshot_recovery_restores_records() {
    ...
    let write_ts = now_ns();
    {
        ...
        for ts in [10u64, 20, 30] {
            unsafe { (*buf).push(lloogg::record::Record { event_type: 1, timestamp: ts, url_hash: 0 }, write_ts) };
        }
